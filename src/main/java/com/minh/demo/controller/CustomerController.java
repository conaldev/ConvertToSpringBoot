...
public class CustomerController {
    ...
    @GetMapping
    public ModelAndView showList(Optional<String> s, @PageableDefault(size = 4) Pageable pageInfo) {
...
//    }
    ...
    public ResponseEntity<Customer> apiUpdateCustomer(@PathVariable("id") long id,
        ...
        Customer originCustomer = customerService.findById(id);
        ...
        if (customer.getProvince() != null) {
            Province originProvince = provinceService.findById(customer.getProvince().getId());
    ...
    public ResponseEntity<Void> apiDeleteCustomer(@PathVariable Long id) {
        System.out.println("Delete coming----");
        Customer target = customerService.findById(id);
